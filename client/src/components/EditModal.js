...
const EditModal = ({EditModal,setEditModal,oneCust,customer,setCustomer,users,setUsers}) => {
    ...
    useEffect(()=>{
        if(oneCust){
            ...
            setEmail(oneCust.email||"");
            setPhone(oneCust.phone||"");
            setCompany(oneCust.company||"");
            setPriority(oneCust.priority||"");
            setStatus(oneCust.status||"");
            setAssignedTo(oneCust.assignedTo?._id||"");
            setLastcontacted(oneCust.lastcontacted
                ? new Date(oneCust.lastcontacted).toISOString().slice(0,16)
                : ""
            );
            setnextFollowup(oneCust.nextFollowup
                ? new Date(oneCust.nextFollowup).toISOString().slice(0,16)
                : ""
            );
        }
    },[oneCust])
